...
class Docs extends Component {
  constructor(props) {
    ...
    this.allowedExtensions = ['.txt', '.csv', '.pdf', '.doc', '.docx', '.xls' , '.xlsx', '.ppt', '.pptx', '.odt', '.ods', '.odp'];
  ...
  async uploadFile(event) {
    ...
    if(this.allowedTypes.indexOf(file.type) >= 0 && file.size <= this.maxSize)
    {
        ...
        var res = await fetch(auth.prepareURL(process.env.REACT_APP_API_URL_TASKS_UPLOAD),
  ...
  render() {
    const {t, docs, editable} = this.props;
      ...
      default:
        return (
                  <>
                    ...
                    {docs.map((item, index) => (
                        <CRow key={"DocsRow-"+ index} style={{background: "whitesmoke", padding: "5px", width: "98%", marginBottom: "5px"}}>
                                    {edit === index
                                      ?<>
